...
def create_dataset(train_df, val_df):
    ...
    train_dataset = Dataset.from_dict(
        {"title": [{"content": source, "title": target} 
                         for source, target in zip(train_df["content"], train_df["title"])]}
    )
    valid_dataset = Dataset.from_dict(
        {"title": [{"content": source, "title": target} 
                         for source, target in zip(val_df["content"], val_df["title"])]}
    )
...
def preprocess_function(examples, tokenizer, prefix, max_length=128, 
    ...
    """
    inputs = [prefix + ex[source] for ex in examples["title"]]
    targets = [ex[target] for ex in examples["title"]]
